...
function setControlData() {
    if(instance.currentStep == "发起流程"){
        ...
        isAbleSelectBranch = instance.currentStep == "科室审批";
    }
    isShowBranchSelect = isAbleSelectBranch;
    if(instance.currentStep == "发起流程" && user.launchRole.name.indexOf("计划科科员")>-1){
...
function initContentInterface() {
    ...
    $("#saveForTableLine").click(function () {
        ...
        var cHandleWay = $("#selectForHandleWay");
        var cHandleForm = $("#selectForHandleForm");
        ...
        if (isCorrect) {
            if (infoEditFor) {
                ...
                cInfo[12].innerText = cHandleWay.val();
                cInfo[13].innerText = cHandleForm.val();
                cInfo[14].innerText = cComment.val();
            } else {
                var info = {};
                ...
                info.handleWay = cHandleWay.val();
                info.handleForm = cHandleForm.val();
    ...
    $("#moreTableLine").click(function () {
        ...
        $("#selectForHandleWay").val(cInfo[12].innerText);
        $("#selectForHandleForm").val(cInfo[13].innerText);
        $("#inputForComment").val(cInfo[14].innerText);
...
$(document).ready(
    function init() {
        initCommon("nationalAssetDisposeOne");
...
function getAll() {
    ...
    for (var i = 0; i < tableList.length; i++) {
        ...
        element.handleWay = cInfo[12].innerText;
        element.handleForm = cInfo[13].innerText;
        element.comment = cInfo[14].innerText;
...
function addBillInfo(info, canOperate) {
    ...
    newTableLine +=
        ...
        "<label class='combine'>处置方式：<span>"+info.handleWay+"</span></label>" +
        "<label class='combine'>处置形式：<span>"+info.handleForm+"</span></label>" +
