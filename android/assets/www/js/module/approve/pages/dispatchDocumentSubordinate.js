...
function getNextStep() {
    if(instance.isOperator){
        switch (instance.currentStep){
            case "发起流程":
                if(user.launchRole.name.indexOf("办公室主任") > -1){
                    instance.nextStep = "核稿（单位负责人审批）";
                }else{
                    instance.nextStep = "核稿（分管领导审批）";
                }
                break;
            case "核稿（单位负责人审批）":
...
function initContentInterface(){
    ...
    initExamineApproveControl(instance.currentStep.indexOf("核稿") > -1,undefined);
    initSignApproveControl(instance.currentStep.indexOf("签发") > -1,timeAdviceExamine);
}

$(document).ready(
    function init() {
        initCommon("dispatchDocumentSubordinate");
