...
function initContentInterface() {
    ...
    $("#saveForTableLine").click(function () {
        ...
        var cBalanceType = $("#selectForBalanceType");
        var cEvidenceIndex = $("#inputForEvidenceIndex");
        var cEvidenceNumber = $("#inputForEvidenceNumber");
        ...
        isCorrect = checkPositiveNum(cEValue.val(), $("#hintForEValue")) && isCorrect;
        isCorrect = checkNotNull(cEvidenceIndex.val(), $("#hintForEvidenceIndex")) && isCorrect;
        isCorrect = checkNotNullPositiveInteger(cEvidenceNumber.val(), $("#hintForEvidenceNumber")) && isCorrect;
        if (isCorrect) {
            if (infoEditFor) {
                ...
                cInfo[13].innerText = cBalanceType.val();
                cInfo[14].innerText = cEvidenceIndex.val();
                cInfo[15].innerText = cEvidenceNumber.val();
                ...
                info.balanceType = cBalanceType.val();
                info.evidenceIndex = cEvidenceIndex.val();
                info.evidenceNumber = cEvidenceNumber.val();
    ...
    $("#moreTableLine").click(function () {
        ...
        $("#selectForBalanceType").val(cInfo[13].innerText);
        $("#inputForEvidenceIndex").val(cInfo[14].innerText);
        $("#inputForEvidenceNumber").val(cInfo[15].innerText);
...
function getAll() {
    ...
    for (var i = 0; i < tableList.length; i++) {
        ...
        element.balanceType = cInfo[13].innerText;
        element.evidenceIndex = cInfo[14].innerText;
        element.evidenceNumber = cInfo[15].innerText;
...
function addBillInfo(info, canOperate) {
    ...
    newTableLine +=
        ...
        "<label class='combine'>原值：<span>"+info.OValue+"</span>元</label>" +
        "<label class='combine'>累计折旧：<span>"+info.PValue+"</span>元</label>" +
        "<label class='combine'>净值：<span>"+info.FValue+"</span>元</label>" +
        "<label class='combine'>评估值：<span>"+info.EValue+"</span>元</label>" +
        "<label class='combine'>损溢类型：<span>"+info.balanceType+"</span></label>" +
        "<label class='combine'>证据索引编号：<span>"+info.evidenceIndex+"</span></label>" +
        "<label class='combine'>证据数量：<span>"+info.evidenceNumber+"</span></label>" +
