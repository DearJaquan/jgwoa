...
/****************************以下内容为流程控制变量****************************/
//可发起角色列表
var canLaunch = ["付款审批单(妇工委)发起人","付款审批单(团委)发起人","付款审批单(机关工会)发起人","付款审批单(委工会)发起人"];
...
function initContentInterface() {
    ...
    if(instance.launcherSection != "领导"){
        ...
        initSectionApproveControl(instance.currentStep == "负责人审批",undefined);//科长审批信息显示控制
...
$(document).ready(
    function init() {
        initCommon("paymentApproveUnion");
