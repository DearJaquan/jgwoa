...
function getContent(){
    ...
    switch (instance.currentStep){
        ...
        case "行政办确认":
            ...
            content.imitateAdvice = cImitateAdvice.val();
...
function initContentInterface(){
    ...
    cTitle.val(title);
    // cImitateAdvice.val(imitateAdvice);
    checkList.forEach(function (value) {
        $("#checkList").append("<li><a class='name'>"+value.name+"</a></li>");
    });
    $("#showPersonSelect").css("display","none");
    if(instance.isOperator && (instance.currentStep == "发起流程" || instance.currentStep == "行政办确认")) {
        ...
        // cImitateAdvice.attr("readonly",false);
        // cImitateAdvice.removeClass("disable");
        // cImitateAdvice.addClass("default");
...
$(document).ready(
    function init() {
        initCommon("documentCircularize");
